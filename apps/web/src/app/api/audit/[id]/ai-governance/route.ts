...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  try {
    const { getAuditById, getAiGovernanceReport } = await import('@sitenexis/db');
    ...
    const report = await getAiGovernanceReport(id);
    const state = resolveGTLState(audit.status, report !== null);
    return gtlResponse(state, report);
  } catch (err) {
    logger.error({ err }, 'GET /api/audit/[id]/ai-governance failed');
    return NextResponse.json({ error: 'Failed to fetch AI governance report' }, { status: 500 });
  }
}
