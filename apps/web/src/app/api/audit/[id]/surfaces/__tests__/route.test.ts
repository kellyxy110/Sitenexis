...
describe('GET /api/audit/[id]/surfaces', () => {
  ...
  it('owner succeeds and reads the persisted surface map', async () => {
    h.getRecommendationSurfaceMap.mockResolvedValueOnce({ overallSurfaceScore: 61 });
    const res = await GET(req(), params);
    const json = await res.json();
    expect(res.status).toBe(200);
    expect(json.data).toMatchObject({ auditId: 'audit-1', overallSurfaceScore: 61 });
