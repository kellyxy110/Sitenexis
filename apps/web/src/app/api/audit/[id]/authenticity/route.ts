...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  try {
    const { getAuditById, getLatestSyntheticEntityAnalysis } = await import('@sitenexis/db');
    ...
    const state = resolveGTLState(audit.status, !!analysis);

    return gtlResponse(state, analysis ? { auditId: id, ...analysis } : null);
  } catch (err) {
