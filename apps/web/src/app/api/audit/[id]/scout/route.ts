...
export async function GET(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  try {
    const { getAuditWithResults, getScoutAnalysis } = await import('@sitenexis/db');
    ...
    const result = await getScoutAnalysis(id);
    const state = resolveGTLState(audit.status, !!result);

    return gtlResponse(state, result);
  } catch (err) {
    logger.error({ err }, 'GET /api/audit/[id]/scout failed');
    return NextResponse.json({ error: 'Failed to load Scout analysis data' }, { status: 500 });
  }
}
