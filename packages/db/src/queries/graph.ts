 ...
 */
export async function writeFactGraph(
  auditId: string,
  pages: CrawledPage[],
): Promise<void> {
  ...
  const schemaEntityNodes: {
    auditId: string; nodeType: string; label: string; url: null;
  ...
  const internalEdges: {
    auditId: string; fromUrl: string; toUrl: string; relationship: string;
    anchorText: string | null; position: string | null; isNoFollow: boolean; confidence: number;
  }[] = [];

  const externalEdges: {
    auditId: string; fromUrl: string; toUrl: string; relationship: string;
    anchorText: null; position: null; isNoFollow: boolean; confidence: number;
  }[] = [];
 ...
 */
export async function writePerceptionGraph(
  auditId: string,
  snapshot: PerceptionGraphSnapshot,
): Promise<void> {
  if (snapshot.nodes.length === 0) return;

  await db.perceptionNode.createMany({
  ...
  await db.perceptionEdge.createMany({
